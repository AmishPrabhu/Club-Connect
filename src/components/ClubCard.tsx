...
export default function ClubCard({ club, onClick, isLiked = false, onToggleLike }: ClubCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-white dark:bg-slate-900 rounded-xl p-4 md:p-6 border border-slate-200 dark:border-slate-800 hover:border-[#002147] dark:hover:border-blue-500 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col h-full"
    >
      ...
      <div className="flex items-start justify-between mb-3 md:mb-4 mt-1">
        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center text-xl md:text-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 shadow-inner flex-shrink-0`}>
          {club.image ? (
            <img src={club.image} alt={club.name} className="w-full h-full object-cover rounded-xl" />
          ) : (
            <span className="text-2xl md:text-3xl">{club.icon}</span>
          )}
        </div>
      </div>

      <h3 className="text-base md:text-xl font-serif font-bold text-slate-900 dark:text-white mb-1.5 group-hover:text-[#002147] dark:group-hover:text-blue-400 transition-colors line-clamp-1">
        {club.name}
      </h3>
      <p className="text-xs md:text-sm text-slate-600 dark:text-slate-300 mb-3 md:mb-4 line-clamp-2 md:min-h-[40px] flex-grow">
        {club.description}
      </p>

      <div className="flex items-center gap-2 mb-3 md:mb-4">
        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide bg-blue-50 dark:bg-blue-900/30 text-[#002147] dark:text-blue-300 border border-blue-100 dark:border-blue-800 truncate">
          {club.category}
        </span>
      </div>

      <div className="flex items-center justify-between pt-3 md:pt-4 border-t border-slate-100 dark:border-slate-800 mt-auto">
        <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 w-full">
          <div className="flex items-center gap-1 text-[10px] md:text-xs font-semibold text-slate-500 dark:text-slate-400">
