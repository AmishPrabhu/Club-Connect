            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 md:p-8 border-b border-slate-200 dark:border-slate-700">
                    <div className="flex items-start gap-3 md:gap-4">
                        <div className={`shrink-0 p-2 md:p-3 rounded-xl bg-white dark:bg-slate-800 shadow-md ${notification.type === 'event' ? 'text-blue-600' :
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                                <span className={`px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider ${notification.type === 'event' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' :
                                ...
                                <span className="flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400">
                                    ...
                                    {notification.createdAt instanceof Date
                                        ? notification.createdAt.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })
                                        : new Date(notification.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })
                                    }
                                </span>
                            </div>
                            <h1 className="text-xl md:text-3xl font-black text-slate-900 dark:text-white leading-tight break-words">
                                {notification.title}
                            </h1>
                        </div>
                    </div>
