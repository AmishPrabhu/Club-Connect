...
export default function EventManagement({ eventId, onBack, user: propUser }: EventManagementProps) {
            ...
            try {
                ...
                if (foundPost) {
                    ...
                    setFormData({
                        ...
                        totalSessions: (foundPost as any).totalSessions || 1,
                    });
    ...
    const handleExportAttendance = async () => {
        ...
        const totalSessions = formData.totalSessions || 1;

        // Set columns - include per-session columns
        ...
        for (let i = 1; i <= totalSessions; i++) {
            columns.push({ header: totalSessions === 1 ? 'Status' : `Session ${i}`, key: `Session_${i}`, width: 15 });
        }

        if (totalSessions > 1) {
            columns.push({ header: 'Overall Status', key: 'Overall', width: 15 });
        }
        ...
        eventRsvps.forEach(rsvp => {
            ...
            const sa = rsvp.sessionAttendance || {};
            for (let i = 1; i <= totalSessions; i++) {
                const status = sa[String(i)] || 'pending';
                row[`Session_${i}`] = status.charAt(0).toUpperCase() + status.slice(1);
            }

            if (totalSessions > 1) {
                row['Overall'] = rsvp.attendance ? rsvp.attendance.charAt(0).toUpperCase() + rsvp.attendance.slice(1) : 'Pending';
            }
        ...
        worksheet.addRow([totalSessions > 1 ? 'Present (All Sessions)' : 'Present', present]);
                                ...
                                <div className="lg:col-span-2 space-y-6">
                                    <div className="bg-white/85 dark:bg-slate-900/80 backdrop-blur-md rounded-xl p-6 shadow-sm border border-slate-200/60 dark:border-slate-700/40">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Number of Sessions</label>
                                                <input
                                                    type="number"
                                                    min={1}
                                                    max={20}
                                                    value={formData.totalSessions}
                                                    onChange={(e) => setFormData({ ...formData, totalSessions: Math.max(1, parseInt(e.target.value) || 1) })}
                                                    className="w-full px-4 py-2 bg-white/85 dark:bg-slate-900/80 backdrop-blur-md border border-slate-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white"
                                                />
                                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                                    Attendance is tracked per session. Certificates require presence in all sessions.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <table className="w-full">
                                        <thead className="bg-slate-50 dark:bg-slate-700/50">
                                            <tr>
                                                ...
                                                {Array.from({ length: formData.totalSessions }, (_, i) => (
                                                    <th key={i} className="text-center px-3 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">
                                                        {formData.totalSessions === 1 ? 'Attendance' : `S${i + 1}`}
                                                    </th>
                                                ))}
                                                {formData.totalSessions > 1 && (
                                                    <th className="text-center px-3 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Overall</th>
                                                )}
                                        ...
                                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                            {(() => {
                                                ...
                                                if (filtered.length === 0 && searchTerm) {
                                                    return (
                                                        <tr>
                                                            <td colSpan={4 + formData.totalSessions + (formData.totalSessions > 1 ? 1 : 0)} className="py-8 text-center text-slate-500 dark:text-slate-400">
                                                ...
                                                return filtered.map(participant => {
                                                    const sa = participant.sessionAttendance || {};
                                                    return (
                                                    <tr key={participant.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                                        ...
                                                        {Array.from({ length: formData.totalSessions }, (_, i) => {
                                                            const sessionKey = String(i + 1);
                                                            const sessionStatus = sa[sessionKey] || 'pending';
                                                            return (
                                                                    ...
                                                                    <div className="flex items-center justify-center gap-1">
                                                                        <button
                                                                            onClick={async () => {
                                                                                ...
                                                                                const success = await updateParticipantAttendance(eventId, participant.id, 'present', i + 1);
                                                                                if (success) {
                                                                                    setEventRsvps(prev => prev.map(p => {
                                                                                        if (p.id !== participant.id) return p;
                                                                                        const newSA = { ...(p.sessionAttendance || {}), [sessionKey]: 'present' as const };
                                                                                        const vals = Object.values(newSA);
                                                                                        const att = vals.every(v => v === 'present') ? 'present' as const : vals.some(v => v === 'absent') ? 'absent' as const : 'pending' as const;
                                                                                        return { ...p, sessionAttendance: newSA, attendance: att };
                                                                                    }));
                                                                                }
                                                                            }}
                                                                            className={`px-2 py-1 rounded text-xs font-bold transition-all ${sessionStatus === 'present'
                                                                        ...
                                                                        <button
                                                                            onClick={async () => {
                                                                                if (!participant.id) return;
                                                                                const success = await updateParticipantAttendance(eventId, participant.id, 'absent', i + 1);
                                                                                if (success) {
                                                                                    setEventRsvps(prev => prev.map(p => {
                                                                                        if (p.id !== participant.id) return p;
                                                                                        const newSA = { ...(p.sessionAttendance || {}), [sessionKey]: 'absent' as const };
                                                                                        const vals = Object.values(newSA);
                                                                                        const att = vals.every(v => v === 'present') ? 'present' as const : vals.some(v => v === 'absent') ? 'absent' as const : 'pending' as const;
                                                                                        return { ...p, sessionAttendance: newSA, attendance: att };
                                                                                    }));
                                                                            ...
                                                                            className={`px-2 py-1 rounded text-xs font-bold transition-all ${sessionStatus === 'absent'
                                                        ...
                                                        {formData.totalSessions > 1 && (
                                                            <td className="px-3 py-3 text-center">
                                                                <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                                                                    ...
                                                                    {participant.attendance === 'present' ? '✓ All' : participant.attendance === 'absent' ? '✗ Missed' : 'Pending'}
                                                                </span>
                                                            </td>
                                                        )}
                                                        <td className="px-4 py-3">
